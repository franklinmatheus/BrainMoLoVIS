...
from inputsubjectwindow import InputSubjectWindow
...
class MonitoringWindow():
    ...
    def subjectid_config(self):
        inputsubject = InputSubjectWindow(self.__subjectid)
        self.__window.wait_variable(inputsubject.get_inputed())
    ...
    def confirm_reset(self):
            ...
            #messagebox.showinfo('Important', 'The monitoring window has been reset.', parent=self.__window)
    ...
    def __init__(self, exportpath) -> None:
        self.__window = Toplevel()
        self.__window.title('Brain Monitor')
        ...
        self.USER = PhotoImage(file = r"./imgs/user.png")
        self.USER_SMALL = PhotoImage(file = r"./imgs/user_small.png")
        ...
        self.ax_raw = fig.add_subplot(grid[0:3,0:8]) # raw
        self.ax_eeg_power = fig.add_subplot(grid[3:,0:8]) # eeg power
        self.ax_esense_at_line = fig.add_subplot(grid[0:3,8:12]) # attention esense line
        self.ax_esense_at_bar = fig.add_subplot(grid[0,13]) # attention esense bar
        self.ax_esense_med_line = fig.add_subplot(grid[3:6,8:12]) # meditation esense line
        self.ax_esense_med_bar = fig.add_subplot(grid[3,13]) # meditation esense bar
        self.ax_blink = fig.add_subplot(grid[-1,8:12]) # blink
        self.ax_esense_at_value = fig.add_subplot(grid[0,12]) # attention value
        self.ax_esense_med_value = fig.add_subplot(grid[3,12]) # attention value
        self.ax_gen_at_line = fig.add_subplot(grid[0:3,-4:]) # custom attention
        self.ax_gen_med_line = fig.add_subplot(grid[3:6,-4:]) # custom meditation
        self.set_charts_states(False)
        ...
        def animate(i):
            ...
            if len(self.__raw_eeg_data.get()) > 0:
                ...
                #self.ax_raw.set_xlim(int(self.__raw_eeg_data.length()/100)*100, int(self.__raw_eeg_data.length()/100)*100+100)
            ...
            if len(self.__attention_esense.get()) > 0:
                ...
                #self.ax_esense_at_line.set_xlim(int(self.__attention_esense.length()/25)*25, int(self.__attention_esense.length()/25)*25+25)
                ...
                #line_atesense.set_ydata(self.__attention_esense.get())
                #line_atesense.set_xdata(np.arange(self.__attention_esense.length()-len(self.__attention_esense.get()), self.__attention_esense.length(), 1))
            ...
            if len(self.__meditation_esense.get()) > 0:
                ...
                #self.ax_esense_med_line.set_xlim(int(self.__meditation_esense.length()/25)*25, int(self.__meditation_esense.length()/25)*25+25)
                ...
                #line_medesense.set_ydata(self.__meditation_esense.get())
                #line_medesense.set_xdata(np.arange(self.__meditation_esense.length()-len(self.__meditation_esense.get()), self.__meditation_esense.length(), 1))
            ...
            if len(self.__attention_gen.get()) > 0:
                ...
                #self.ax_gen_at_line.set_ylim(0,100)
            ...
            if len(self.__meditation_gen.get()) > 0:
                ...
                #self.ax_gen_med_line.set_ylim(0,100)
            ...
            if len(self.__attention_esense.get()) > 0:
                #self.ax_esense_at_value.text(0, 0, 'Attention', color='red', size=6)
            ...
            #self.ax_esense_med_value.text(0, 0, 'Meditation', color='blue', size=6)
            if len(self.__meditation_esense.get()) > 0:
        ...
        # TOP FRAME
        topframe = Frame(self.__window, padx=70, pady=20, bg='white')
        ...
        Label(topframe, image=self.USER, bg='white').pack(side='left')
        Hovertip(self.__subjectid_label, 'Subject ID.')

        #self.__config_button = Button(topframe, text='Configure', image=self.CONFIG, command=self.subjectid_config)
        ...
        # MIDDLE FRAME
        midframe = Frame(self.__window)
        midframe.pack(expand=True, fill='both')

        canvas = FigureCanvasTkAgg(fig, master=midframe)
        canvas.draw()
        ...
        # BOTTOM FRAME
        botframe = Frame(self.__window, padx=5, pady=5)
        botframe.pack(fill='x', side='bottom')

        self.__config_button = Button(botframe, text='Configure', image=self.USER_SMALL, command=self.subjectid_config)
        self.__config_button.pack(side='left', padx=5)
        Hovertip(self.__subjectid_label, 'Set the Subject ID.')
        ...
        #self.__check_show_gen_values = Checkbutton(botframe, text='Display generated values', variable=self.__show_gen_values, onvalue=1, offvalue=0)
        #self.__check_show_gen_values.pack(side='left',padx=5)
